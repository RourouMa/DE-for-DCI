...
若实际约化表达式的全部 G 都通过 `FiniteIntegralQ`，直接以这些 G 为覆盖，不再计算最小行空间、候选成员资格或有限组合。以线性系数映射精确保留 weights 和所有 `BoundaryIntegral` 源项。有非单独有限支撑时才进入有限组合搜索，优先构造并认证简单常系数组合。这是默认构造器的搜索策略，不是禁止变量系数；扩展策略与每轮汇报须遵守 [汇报与选基规范](REPORTING.zh-CN.md)。全有限支持的直接覆盖也不等于最小独立基，不能用其元素数替代实际空间秩。
