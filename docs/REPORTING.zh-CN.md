...
2026-09-25 用户补充：每次重放均逐轮列出不可约积分数的变化。操作上列出当前关系池下的同圈输入有理秩、输入加双向导数秩及前次→本次增减量，有限基项数另列。比较同一轮此前最近一次已验证约化；尚未重放到的轮次标为待重放。约化通过但有限覆盖未通过的轮次仍保留秩，并明确下一轮基未接受。该比较不是全局主积分最小性证书。
...
若已有 critical-point 或 Euler-characteristic 计数，另报其范围、认证状态和适用的秩上界；对比实际输入秩及输入加导数秩。超过认证上界时优先检查关系缺口及计数口径，不继续无条件扩大基。计数未完成就明确写未知，不把现有 DE 基数倒填成几何计数。详见 [几何计数与终止策略](MASTER_COUNT_BOUND.zh-CN.md)。

计数须按圈数分开，最高圈先报模去低圈后的上限；保留全部低圈源项，不把投影解释成删除。未计算的低圈写未知，不填零。紧 top 计数试验还须报告 ansatz 阶数、耗时及是否触及预算；区分无已发现关系、未约束全部方向、代数运算超时，不统称“没有有限上限”。

2026-09-25 起，本次四圈 ladder 使用用户估定阈值 100。每轮报告 `ThresholdOrigin=UserEstimate`、`StoppingThreshold=100`、输入秩及输入加双向导数秩。超过阈值时报告补关系／重放，不报告下一轮基已经接受；100 不标为数学认证上界，低圈源项继续完整保留。

本次任务严格先做最高圈闭合，期间只保留低圈源项的符号与系数，不并行求解低圈。最高圈模低圈源项的闭合已验证后，再收集最终所需的全部源项，统一做低圈约化、闭合和完整系统验证。

## 复杂度审查（0.3.2）

每轮记录候选及规范形的 r/s/dots、sector、最大幂次和实际 rank。出现异常复杂代表时，报告属于未查询、已有简单约化可换基，还是残留复杂支撑需补关系。`ComplexityReviewRequired` 是待修复状态，不是新一轮或闭合结果。扩充实验同时报告复杂度变化和输入＋DE rank；单点筛查必须与符号重构及残差验证区分。详见 [复杂度指导撒点](COMPLEXITY_GUIDED_SEEDING.zh-CN.md)。
