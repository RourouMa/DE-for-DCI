...
**当前入口（2026-09-25）：** 本文是旧档案交接记录。请先读 [当前英文交接说明](COLLABORATOR_HANDOFF.md)，采用 0.5.0 启动器和修正后的三圈结果；不要按本文旧的私有仓库、版本或旧池迁移建议开展新计算。

**2026-09-21 复现补充：** 第四轮 66 维旧生产结果现有独立的[固定快照与 Ubuntu 复现入口](../reproduction/ordering01/README.zh-CN.md)。完整矩阵、撒点和组合在同一私有仓库的 Release assets，须运行 `fetch.py --all` 下载；不能只运行新 package 期待自动复现旧计数。下面关于“大数据尚未随仓库提供”的内容是 2026-09-20 的历史状态，第四轮所需紧凑数据已由上述快照补齐；全部历史中间缓存仍未整体上传。
...
7. **闭合前提下，优先简单常系数有限组合。** 优先两项、小支撑、小整数系数；常系数是强优先项，并非绝对禁令。三圈已知存在常系数解，因此搜索遇阻时先核查候选空间、约化及 supersector 的 IBP/symmetry；四圈不能预设常系数一定足够。必要时自主比较变量系数方案，记录常系数搜索范围、关系审计、替代方案及选用理由，并完整求导系数、验证有限性和精确覆盖。不能任意猜 `Hi-H1`，零和本身不证明发散抵消。详见 [汇报与选基规范](REPORTING.zh-CN.md)。
