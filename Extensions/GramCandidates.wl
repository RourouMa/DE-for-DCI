...
GramCandidatePolynomials[f_Association,OptionsPattern[]]:=Module[{v=Join[f["External"],f["Loops"]],dim=f["Dimension"]+2,sets,pairs,m,result={},poly,terms,degree,actual},
 ...
 If[actual=!={degree},Return[Failure["GramDegree",<|"Pair"->pair,"Expected"->degree,"Actual"->actual|>]]];
 AppendTo[result,<|"FamilyHash"->f["Hash"],"EmbeddingDimension"->dim,"Rows"->pair[[1]],"Columns"->pair[[2]],"LoopDegree"->degree,"Terms"->terms,"Polynomial"->poly,"Proof"->"Every (D+3)-minor of V.metric.Transpose[V] vanishes in embedding dimension D+2","DimensionSpecific"->True|>],{pair,pairs}];result];
Options[GenerateGramCandidateRelations]={"MaxSeedsPerCenter"->1,"ProgressFunction"->None};
GenerateGramCandidateRelations[f_Association,centers_List,polynomials_List,OptionsPattern[]]:=Module[{gs=ConformalIBP`Private`support[centers],limit=OptionValue["MaxSeedsPerCenter"],report=OptionValue["ProgressFunction"],records={},seen=<||>,powers,cs,candidates,atoms,expr,h,n=0,accepted,a,deg},
 If[!IntegerQ[limit] || limit<1,Return[Failure["SeedLimit",<||>]]];
 If[!And@@(#["FamilyHash"]===f["Hash"]& /@ polynomials),Return[Failure["FamilyMismatch",<||>]]];
 ...
 expr=ConformalIBP`Private`canonExpr[f,cs.atoms];If[FailureQ[expr],Return[expr]];If[ConformalIBP`Private`zero[expr],Continue[]];
 ...
 <|"FamilyHash"->f["Hash"],"Candidates"->records,"Equations"->(#["Relation"]& /@ records),"CompleteSeedEnumeration"->False,"MaxSeedsPerCenter"->limit,"AutomaticallyMerged"->False|>];
End[];EndPackage[];
