...
AssessBasisComplexity[f_Association,basis_List,reduction_Association,OptionsPattern[]]:=Module[
 {atoms=support[basis],profiles,known=Association[Lookup[reduction,"Rules",{}]],sectorProfiles=OptionValue["SectorProfiles"],suspect,records,flagged,nfTargets,unqueried},
 ...
 records=Table[Module[{cg=CanonicalIntegral[f,g],nf,hard,seen},
  If[FailureQ[cg],Return[cg]];seen=KeyExistsQ[known,cg];nf=If[seen,known[cg],cg];
  ...
  Join[IntegralComplexity[f,g],<|"Canonical"->cg,"Flagged"->suspect[g],"Queried"->seen,
   ...
   "Classification"->Which[!seen,"QueryBeforeJudging",hard=!={},"TargetedRelationAudit",suspect[g],"ReplaceComplexRepresentative",True,"NoComplexitySignal"]|>]],{g,atoms}];
 ...
 "NextAction"->Which[nfTargets=!={},"Target inverse operator shifts at complex normal forms; compare simpler support and DE ranks",unqueried=!={},"Query flagged candidates in the existing pool before generating equations",flagged=!={},"Select a simpler finite representative using existing reductions",True,"Proceed with independently certified finite coverage"]|>];
