...
BuildSpanPreservingFiniteBasis[f_Association,rows_List,queries_List,reduction_Association]:=Module[
 ...
 cs=Union[raw,support[queries],Select[queries,Length[support[#]]>1&&constantCombinationQ[#]&],If[AssociationQ[baseline],baseline["Basis"],{}]];
 ...
 images=canonicalLinear /@ ((canonExpr[f,#]& /@ cs)/.Dispatch[Normal[known]]);
 eligible=Select[Range[Length[cs]],Complement[support[images[[#]]],raw]==={}&&With[{v=First[coeff[{images[[#]]},raw]]},AllTrue[v-v[[p]].r,zero]]&];
 ...
 ec=coeff[images[[eligible]],raw];ind=spanFFRowSpace[Transpose[ec]];If[FailureQ[ind],Return[ind]];ind=pivots[ind];
 ...
 w=coeff[wi,bv];res=canonicalLinear /@ (rows-w.bi);If[!AllTrue[res/._BoundaryIntegral->0,zero],Return[fail["SpanCoverage","Exact coverage residuals are nonzero."]]];
...
campaignFiniteCover[f_,rows_,queries_,reduction_,policy_]:=If[policy==="PreserveActualSpan",BuildSpanPreservingFiniteBasis[f,rows,queries,reduction],BuildFiniteBasis[f,rows]];
